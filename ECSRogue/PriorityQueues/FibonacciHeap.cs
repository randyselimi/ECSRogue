...
namespace ECSRogue.PriorityQueues
{
    public sealed class FibonacciHeap<TItem, TPriority> : IPriorityQueue<TItem, TPriority>
    {
        ...
        public TItem Peek
        {
            get
            {
                if (Count == 0) throw new InvalidOperationException("Heap contains no elements");
                return head.Item;
            }
        }
        ...
        {
            foreach (var entry in Enumerate()) yield return entry.Item;
        }
        ...
        public IPriorityQueueEntry<TItem> Enqueue(TItem item, TPriority priority)
        {
            ...
            var node = new FibonacciNode(item, priority, identifier);
            ...
            else
            {
                ...
                if (Compare(head.Priority, priority) > 0) head = node;
            }
        ...
        public void UpdatePriority(IPriorityQueueEntry<TItem> entry, TPriority priority)
        {
            ...
            if (priority == null) throw new ArgumentNullException("priority");
            var node = entry as FibonacciNode;
            ...
            if (node.Parent != null && Compare(node.Parent.Priority, priority) > 0) CutNode(node);
        ...
        public TItem Dequeue()
        {
            ...
            while (head.Degree > 0) CutNode(head.FirstChild);
            Concatenate();
        ...
        public void Remove(IPriorityQueueEntry<TItem> entry)
        {
            ...
            var temp = entry as FibonacciNode;
        ...
        public void Clear()
        {
            foreach (var entry in Enumerate()) entry.HeapIdentifier = Guid.Empty;
            Count = 0;
            head = null;
        }
        ...
        private IEnumerable<FibonacciNode> Enumerate()
        {
            ...
            do
            {
                foreach (var node in EnumerateBranch(current)) yield return node;
        ...
        {
            var h = head.Right;
            for (var node = head.Right.Right; node != head.Right; node = node.Right)
        ...
        private void Concatenate()
        {
            ...
            for (var node = head.Right; node != head;)
            {
                var next = node.Right;
                var cont = true;
                do
        ...
        {
            public int Degree;
            public FibonacciNode FirstChild;
